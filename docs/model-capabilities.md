...
The capability registry and Constable controls cover every request parameter documented for the targets below. Unsupported fields are intentionally rejected per target rather than being forwarded to a provider.
...
<!-- prettier-ignore -->
| Target | Modes | Normalized parameters | Output | Seed range |
| --- | --- | --- | --- | --- |
| GPT Image 2 | Text to image, reference-assisted generation | `prompt`, `negative_prompt` (harness-only), optional `image`, `size`, `quality`, `background`, `output_format`, `n` | JPEG, PNG | Not supported |
| GPT Image 2 Edit | Source, optional references and mask | `prompt`, `negative_prompt` (harness-only), `image`, `mask`, `size`, `quality`, `background`, `input_fidelity`, `output_format`, `n` | JPEG, PNG | Not supported |

GPT Image has no native negative-prompt API parameter. Following the ordinary-language exclusions in OpenAI's prompting guide and public GPT Image 2 examples, the harness appends a blank line and `Avoid: <trimmed negative prompt>` to the positive prompt at the server-side provider boundary. This applies to text-only Create, reference-assisted Create, and Edit. Empty or whitespace-only negative prompts leave the provider prompt unchanged, and `negative_prompt` is never sent as an API field. The normalized request and sidecar keep positive and negative prompts separate; provider metadata records the combined `effectivePrompt` when used. Each input retains the harness's 10,000-character limit, so even both maximum-length fields plus the suffix remain below the documented 32,000-character GPT Image prompt limit. Exclusions are instructions to the model, not a guarantee.
...
- Source images remain in the main image area; temporary references form a floating, tilted vertical stack peeking beyond the right edge. Previews have equal heights capped at 180 px, natural aspect-ratio widths, and individual unboxed top-left X controls, without captions, frames, or rounded corners. The stack scrolls vertically and does not move the prompt or working images. Empty inputs never reserve image space. Style-guide references stay in the left-hand stack and can be excluded from requests through the guide panel without deleting their files. Stability targets get only their supported source, mask, and Style Transfer references. GPT Image targets retain reference order and a 16-image total limit, reserving one image for the source in Edit. Extra uploads are reported rather than silently submitted; oversized style guides block generation until excess references are removed. Text-only models and GPT Image 2 Create keep loaded output previews separate from inputs.
...
- [OpenAI Images API parameters and prompt limits](https://developers.openai.com/api/reference/resources/images)
- [OpenAI GPT Image prompting guide: explicit constraints and exclusions](https://developers.openai.com/cookbook/examples/multimodal/image-gen-models-prompting-guide#2-prompting-fundamentals)
- [fal GPT Image 2 prompting examples: Constraints and Avoid sections](https://fal.ai/learn/tools/prompting-gpt-image-2)
- [Community GPT Image 2 prompt collection with ordinary-language exclusions](https://github.com/BackToNull/awesome-gpt-image-2-prompts)
